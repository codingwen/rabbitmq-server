...
-spec(recover() -> [name()]).
-spec(callback(rabbit_types:exchange(), fun_name(),
               fun((boolean()) -> non_neg_integer()) | atom(), [any()]) -> 'ok').
-spec(policy_changed(rabbit_types:exchange(), rabbit_types:exchange()) -> 'ok').
-spec(declare(name(), type(), boolean(), boolean(), boolean(),
              rabbit_framing:amqp_table())
      -> rabbit_types:exchange()).
-spec(check_type(binary()) -> atom() | rabbit_types:connection_exit()).
-spec(assert_equivalence(rabbit_types:exchange(), atom(), boolean(), boolean(), boolean(),
                         rabbit_framing:amqp_table())
      -> 'ok' | rabbit_types:connection_exit()).
-spec(assert_args_equivalence(rabbit_types:exchange(), rabbit_framing:amqp_table())
      -> 'ok' | rabbit_types:connection_exit()).
-spec(lookup(name()) -> rabbit_types:ok(rabbit_types:exchange()) |
                        rabbit_types:error('not_found')).
-spec(lookup_or_die(name()) -> rabbit_types:exchange() |
...
-spec(info(rabbit_types:exchange(), rabbit_types:info_keys())
      -> rabbit_types:infos()).
...
-spec(delete(name(),  'true') -> 'ok' | rabbit_types:error('not_found' | 'in_use');
          (name(), 'false') -> 'ok' | rabbit_types:error('not_found')).
-spec(validate_binding(rabbit_types:exchange(), rabbit_types:binding())
      -> rabbit_types:ok_or_error({'binding_invalid', string(), [any()]})).
-spec(maybe_auto_delete(rabbit_types:exchange(), boolean())
